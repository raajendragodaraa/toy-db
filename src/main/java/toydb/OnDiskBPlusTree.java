...
public class OnDiskBPlusTree {
    ...
    public OnDiskBPlusTree(DiskStorageManager storage, int cacheCapacity) throws IOException {
        ...
        this.degree = storage.getConfig().getBTreeDegree();
        this.maxKeys = this.degree - 1;
    ...
    public BinaryNode.Pointer search(int key) throws IOException {
        ...
        while (true) {
            ...
            if (BinaryNode.NodeFlags.isLeaf(flag)) {
                BinaryNode.LeafNodeData leaf = BinaryNode.LeafNodeData.deserialize(nodeBytes, degree);
                ...
                BinaryNode.InternalNodeData internal = BinaryNode.InternalNodeData.deserialize(nodeBytes, degree);
                int idx = 0;
    ...
    public synchronized void insert(int key, BinaryNode.Pointer dataPointer) throws IOException {
        ...
        BinaryNode.LeafNodeData leaf = BinaryNode.LeafNodeData.deserialize(leafBytes, degree);
        ...
        if (keys.size() <= maxKeys) {
            BinaryNode.LeafNodeData updatedLeaf = new BinaryNode.LeafNodeData(
                    degree, leaf.isRoot, keys, dataPtrs, leaf.prevSibling, leaf.nextSibling
            );
        ...
        BinaryNode.LeafNodeData rightLeaf = new BinaryNode.LeafNodeData(
                degree, false, rightKeys, rightDataPtrs, leafPtr, leaf.nextSibling
        );
        ...
        if (leaf.nextSibling != null) {
            ...
            BinaryNode.LeafNodeData oldNext = BinaryNode.LeafNodeData.deserialize(oldNextBytes, degree);
            BinaryNode.LeafNodeData updatedOldNext = new BinaryNode.LeafNodeData(
                    degree, oldNext.isRoot, oldNext.keys, oldNext.dataPointers, rightPtr, oldNext.nextSibling
            );
        ...
        BinaryNode.LeafNodeData updatedLeftLeaf = new BinaryNode.LeafNodeData(
                degree, false, leftKeys, leftDataPtrs, leaf.prevSibling, rightPtr
        );
        ...
        if (leafWasRoot) {
            ...
            BinaryNode.InternalNodeData newRoot = new BinaryNode.InternalNodeData(degree, true, rootKeys, rootChildren);
        ...
        for (int i = path.size() - 1; i >= 0; i--) {
            ...
            BinaryNode.InternalNodeData parent = BinaryNode.InternalNodeData.deserialize(parentBytes, degree);
            ...
            if (pKeys.size() <= maxKeys) {
                BinaryNode.InternalNodeData updatedParent = new BinaryNode.InternalNodeData(degree, parent.isRoot, pKeys, pChildren);
            ...
            BinaryNode.InternalNodeData rightInternal = new BinaryNode.InternalNodeData(degree, false, rightPKeys, rightPChildren);
            ...
            BinaryNode.InternalNodeData updatedLeftParent = new BinaryNode.InternalNodeData(degree, false, leftPKeys, leftPChildren);
            ...
            if (parentWasRoot) {
                ...
                BinaryNode.InternalNodeData newRoot = new BinaryNode.InternalNodeData(degree, true, newRootKeys, newRootChildren);
                byte[] newRootBytes = newRoot.serialize();
                BinaryNode.Pointer newRootPtr = storage.writeNewNode(newRootBytes);
                cache.putCached(newRootPtr.position, newRootBytes);
